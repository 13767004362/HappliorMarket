...
public class DownloadDB {
     ...
     * 查询最近的待下载项
     *
     * @param packageName 消息类型，为0时，表示不限制类型a
     * @return 最近的待下载项
     */
    public synchronized DownloadInfo queryByPackageName(String packageName) {
        ...
        try {
            ...
            Log.e(TAG, "queryByPackageName# Exception=", e);
            // e.printStackTrace();
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public synchronized List<DownloadInfo> queryAll() {
        ...
        try {
            ...
            Log.e(TAG, "queryAll# Exception=", e);
            // e.printStackTrace();
        } finally {
            if (cursor != null) {
                cursor.close();
